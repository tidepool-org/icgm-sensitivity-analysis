...
metadata_file = "../data/raw/metadata/sample-metadata.csv"
export_folder = "../data/interim/"
...
if __name__ == "__main__":
    # Start Pipeline
    print("Starting batch processing on " + str(len(file_list)) + " files.")
    ...
    results_export_filename = \
        'sample-batch-icgm-condition-stats.csv'

    # Append metadata to each file
    metadata_df = pd.read_csv(metadata_file)
    icgm_condition_summary_df = pd.merge(icgm_condition_summary_df,
                                         metadata_df,
                                         how='left',
                                         on='filename')
    results_path = export_folder + results_export_filename
    icgm_condition_summary_df.to_csv(results_path, index=False)
